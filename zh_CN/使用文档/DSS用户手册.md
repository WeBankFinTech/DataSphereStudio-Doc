...
## 1. 前言
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;本文是DSS1.0.0的简单使用文档，由于篇幅有限，只能涵盖DSS的基本使用流程，更多的操作使用细节，需要用户自行探索，在使用的过程中有任何问题和建议，可以随时联系微众银行大数据平台的相关社区开源人员，我们致力于打造更好的一站式大数据套件，为大数据开源生态贡献自己的力量。  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;同时需要感谢天翼云大数据平台的鼎力协助，为DSS量身设计了UI视觉稿，让DSS变得更好。  

## 2. 登录首页
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;为了方便用户使用，系统默认通过使用Linkis的部署用户名进行登录，如hadoop部署的可以直接通过 用户：hadoop，密码：hadoop(密码就是用户名)登录。 首先输入前端容器地址：192.168.xx.xx:8888 接着输入用户名密码：hadoop hadoop。  
...
## 3. 工作空间管理  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;进入到工作空间页面，可以创建和管理工作空间。工作空间是DSS最顶层的概念，如果用比较直观的例子介绍，工作空间可以是一个部门，一个业务线，或者是一个组织，用来管理数据应用，包括人员，工程，或者组件等，如下图是工作空间首页。
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;在进入相应的工作空间后，会跳转到工程首页，在工程首页，可以创建工程，在实际的开发生产中，工程往往被用来管理开发一类数据应用，工程可以是一个实际的一类数据应用，包括工作流，单任务等，每个工作空间下的工程互相隔离。  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;可以在工程首页创建和管理工程，包括创建，修改，和删除，同时在工程首页，可以在顶部切换对应的工作空间，菜单的工作空间管理可对工作空间进行管理。
![工程首页](../Images/projectpage.png)
<center>图4.1 工程首页</center>  
...
# 5. DSS应用组件
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;在工程首页，可以点击进入DSS应用商店，应用商店页面，提供了访问接入DSS第三方组件的快捷方式，如数据服务，Scritpis，Visualis，Schedulis等。

## 5.1. Scripits
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Scriptis是微众银行微数域(WeDataSphere)打造的一站式交互式数据探索分析工具，以任意桥(Linkis)做为内核，提供多种计算存储引擎(如Spark、Hive、TiSpark等)、Hive数据库管理功能、资源(如Yarn资源、服务器资源)管理、应用管理和各种用户资源(如UDF、变量等)管理的能力，Scriptis按功能分为5个功能模块。
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;新建工作流项目后，进入工作流开发页面，通过拖拽，可以实现数据应用的开发。  

![工作流页面](../Images/workflow.png)
<center>图5.2 工作流页面</center>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当双击工作流节点，便进入节点编辑页面，在编辑页面完成脚本开发。  

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当开发完成工作流节点，单击保存，DSS将会保存对应的脚本信息，关闭对应的脚本开发页面，点击编辑模式，重新回到工作流开发界面，为了验证脚本的正确性，建议在开发完成对应的脚本文件后，点击执行，对脚本进行正确性测试，同时也可以在工作流页面，点击执行工作流，完成运行一次工作流任务。  

![执行工作流](../Images/runworkflow.png)
<center>图5.3 执行工作流</center>

&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;当完成工作流开发，点击参数，配置工作流调度配置，通过点击发布按钮，可以一键发布工作流到Schedulis执行。
## 5.3. 数据服务
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;DSS1.0以插件的形式提供数据服务功能，可以方便的实现不具有库表权限的用户，访问特定数据，发布数据服务的用户只需要简单的书写Spark SQL查询，预留查询接口，发布为数据服务，即可让其他用户使用。
